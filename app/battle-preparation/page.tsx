...
export default function BattlePreparation() {
    ...
    const { socket, playerMap, setPlayerMap, myRoomId } = useSocketContext();
    ...
    useEffect(() => {
        ...
        const handleTeamSelected = (data: any) => {
          console.log("Teams updated:", data);
          setTeams(data);
        };
      
        const handleGoToBattle = (roomId: string) => {
          localStorage.setItem("battleTeams", JSON.stringify(teams));
          console.log("Going to battle with teams:", teams);
          router.push("/battle-page");
        };
      
        socket.on("playerJoined", handlePlayerJoined);
        socket.on("teamSelected", handleTeamSelected);
        socket.on("goToBattle", handleGoToBattle);
      
        // ✅ cleanup on unmount or re-render
        return () => {
          socket.off("playerJoined", handlePlayerJoined);
          socket.off("teamSelected", handleTeamSelected);
          socket.off("goToBattle", handleGoToBattle);
        };
      }, [socket, teams, router, setPlayerMap]);
      

    const handleOpenModal = (player: string) => {
        if (player !== playerMap[socket?.id as string]) {
            return;
        }
    ...
    const handleConfirmSelection = (player: string, characters: any[]) => {
        ...
        console.log("Updated Teams:", updatedTeams, playerMap);
      
        socket?.emit("teamSelected", { teams: updatedTeams, roomId: myRoomId }); 
        setOpenModal(false);
      };
